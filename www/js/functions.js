...
/* Welcome */
$(document).delegate("#welcomePage", "pageinit", function () {
    var d = giveN.getResearcherInfo();
    if (d != null) {
        $('#researcherName').val(d[0]);
        $('#researcherEmail').val(d[1]);
        $('#zip').val(d[2]);
    }
...
/*	    $.ajax({
	...
	storeResearcherInfo: function(researcherName, researcherEmail, zip){
	    if (researcherName.length == 0 || researcherEmail.length == 0) {
	        alert("Please enter the person's name and email to send collected data to.");
	        return false;
	...
	getResearcherInfo: function(){
	    var data = this.getTrialData();
	    if (data != null) {
	        return [data.researcherName, data.researcherEmail, data.zip];
	    } else return null;
	},
